...
3. Classify each unresolved item:
   ...
   - Keep an **assumption** labeled with its evidence, `low`/`medium`/`high` confidence, and impact if wrong until it is confirmed or verified.
   - For material items only, keep a compact ledger with evidence for facts; rationale and accepted trade-offs for decisions; and relevant dependencies, consequences, unknowns, and deferred items.
